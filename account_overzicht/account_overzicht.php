<?php
session_start();
require_once '../config/db_connect.php';

// Haal accounts op
try {
    $stmt = $pdo->query("
        SELECT g.Id, Voornaam, Tussenvoegsel, Achternaam, Gebruikersnaam, g.Isactief, g.Datumaangemaakt, r.Naam AS Rol
        FROM Gebruiker g
        LEFT JOIN Rol r ON g.Id = r.GebruikerId
        ORDER BY g.Datumaangemaakt DESC
    ");
    $gebruikers = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("<div class='alert alert-danger'>Fout bij ophalen van accounts: " . htmlspecialchars($e->getMessage()) . "</div>");
}
?>
<!DOCTYPE html>
<html lang="nl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Accountoverzicht</title>

    <!-- Bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- FontAwesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css">

    <style>
        .actieknop {
            padding: 6px 12px;
            margin: 2px;
            border: none;
            border-radius: 4px;
            cursor: pointer;
        }
        .wijzig-knop {
            background-color: #0d6efd;
            color: white;
        }
        .verwijder-knop {
            background-color: #dc3545;
            color: white;
        }
        .actieknop:hover {
            opacity: 0.85;
        }
    </style>
</head>
<body class="bg-light">

<nav class="navbar navbar-expand-lg navbar-dark bg-primary mb-4">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">Accountbeheer</a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#hoofdMenu">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="hoofdMenu">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item">
                    <a class="nav-link active" href="../inloggen/dashboard.php"><i class="fa fa-home"></i> Dashboard</a>
                </li>
                <li class="nav-item"><a class="nav-link" href="#">Instellingen</a></li>
                <li class="nav-item"><a class="nav-link text-warning" href="../inloggen/logout.php">Uitloggen</a></li>
            </ul>
        </div>
    </div>
</nav>

<div class="container py-4">
    <h1 class="text-center mb-4">Accountoverzicht</h1>

    <input type="text" id="zoekInput" class="form-control mb-3" placeholder="Zoek op naam of gebruikersnaam...">

    <?php if (isset($_SESSION['actie_succes'])): ?>
        <div class="alert alert-success"><?= htmlspecialchars($_SESSION['actie_succes']); unset($_SESSION['actie_succes']); ?></div>
    <?php endif; ?>

    <?php if (isset($_SESSION['actie_fout'])): ?>
        <div class="alert alert-danger"><?= htmlspecialchars($_SESSION['actie_fout']); unset($_SESSION['actie_fout']); ?></div>
    <?php endif; ?>

    <?php if ($gebruikers): ?>
        <div class="table-responsive">
            <table class="table table-bordered table-hover bg-white">
                <thead class="table-light">
                    <tr>
                        <th>#</th>
                        <th>Naam</th>
                        <th>Gebruikersnaam</th>
                        <th>Rol</th>
                        <th>Actief</th>
                        <th>Aangemaakt op</th>
                        <th>Acties</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($gebruikers as $gebruiker): ?>
                        <tr>
                            <td><?= htmlspecialchars($gebruiker['Id']) ?></td>
                            <td><?= htmlspecialchars(trim($gebruiker['Voornaam'] . ' ' . $gebruiker['Tussenvoegsel'] . ' ' . $gebruiker['Achternaam'])) ?></td>
                            <td><?= htmlspecialchars($gebruiker['Gebruikersnaam']) ?></td>
                            <td><?= htmlspecialchars($gebruiker['Rol'] ?? 'Onbekend') ?></td>
                            <td><?= $gebruiker['Isactief'] ? 'Ja' : 'Nee' ?></td>
                            <td><?= htmlspecialchars($gebruiker['Datumaangemaakt']) ?></td>
                            <td>
                                <form method="post" action="verwerk_account_acties.php" class="d-inline">
                                    <input type="hidden" name="gebruiker_id" value="<?= $gebruiker['Id'] ?>">
                                    <select name="nieuwe_rol" class="form-select form-select-sm d-inline w-auto">
                                        <option value="Lid">Lid</option>
                                        <option value="Medewerker">Medewerker</option>
                                        <option value="Admin">Admin</option>
                                    </select>
                                    <button type="submit" name="rol_wijzigen" class="actieknop wijzig-knop btn btn-sm">Wijzig</button>
                                </form>
                                <form method="post" action="verwerk_account_acties.php" class="d-inline" onsubmit="return confirm('Weet je zeker dat je dit account wilt verwijderen?');">
                                    <input type="hidden" name="gebruiker_id" value="<?= $gebruiker['Id'] ?>">
                                    <button type="submit" name="verwijderen" class="actieknop verwijder-knop btn btn-sm">Verwijder</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>

        <div class="d-grid mt-4">
            <a href="../inloggen/dashboard.php" class="btn btn-secondary">
                <i class="fa fa-arrow-left"></i> Terug naar dashboard
            </a>
        </div>
    <?php else: ?>
        <div class="alert alert-warning">Geen accounts gevonden.</div>
    <?php endif; ?>
</div>

<script>
    // Zoekfunctie
    document.getElementById("zoekInput").addEventListener("keyup", function () {
        const filter = this.value.toLowerCase();
        const rows = document.querySelectorAll("tbody tr");

        rows.forEach(row => {
            const tekst = row.textContent.toLowerCase();
            row.style.display = tekst.includes(filter) ? "" : "none";
        });
    });
</script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js"></script>

</body>
</html>
